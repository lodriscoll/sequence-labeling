...
def decode(input_length, tagset, score):
    ...
    viterabi_matrix = [[0] * num_tags for _ in range(input_length)]
    backpointers = [[0] * num_tags for _ in range(input_length)]

    # Initialize the start state
    for i in range(num_tags):
        viterabi_matrix[1][i] = score(tagset[i], "<START>", 1)

    # Forward pass
    for t in range(2, input_length - 1):

        for cur_tag_index, cur_tag in enumerate(tagset):

          max_score = float("-inf")
          max_index = -1

          for prev_tag_index, prev_tag in enumerate(tagset):

            current_score = viterabi_matrix[t-1][prev_tag_index] + score(cur_tag, prev_tag, t)

            if current_score > max_score:
                max_score = current_score
                max_index = prev_tag_index

            viterabi_matrix[t][cur_tag_index] = max_score
            backpointers[t][cur_tag_index] = max_index

    # Backward pass
    best_sequence = [""] * input_length
    best_sequence[-1] = "<STOP>"
    best_score = float("-inf")
    best_index = -1

    for i in range(num_tags):
        if viterabi_matrix[input_length-1][i] > best_score:
            best_score = viterabi_matrix[input_length-1][i]
            best_index = i
    best_sequence[input_length-2] = tagset[best_index]

    for t in range(input_length-2, 0, -1):
        best_sequence[t] = tagset[best_index]
        best_index = backpointers[t][best_index]

    best_sequence[0] = "<START>"
...
test_decoder()  
main_predict('ner.test', 'model.simple')
main_train()
